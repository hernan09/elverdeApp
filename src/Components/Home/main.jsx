...
function Home(){
    ...
    useEffect(()=>{
        ...
        .then(json => {
            setData(json);
            setShellOfiAfter(json[2].value_sell)
            setShellBlueAfter(json[3].value_sell)
            setDolarBLue(json[1].source);
            setShellBlue(json[1].value_sell);
            setBuyBlue(json[1].value_buy);
            setDolarOfi(json[0].source);
            setShellOfi(json[0].value_sell);
            setBuyOfi(json[0].value_buy);
        ...
        <div>
           ...
           {!isLoading && <div className='home-contain'>
                <div className='content-dolarblue'>
                    <h3 className='title'>Dólar {DolaBlue}</h3>
                    ...
                    <div ref={divBlueRef} className={`content-text promedio ${flagBlue ? 'higtgreen' : 'lowred'}`}>
                        <h3 className='title-prom'>Promedio</h3>
                        <span className='item-span-prom'>{<CountUp end={averageBlue} duration={0.7}/>}</span>
                        <span className={`content-arrow-blue ${flagBlue ? 'trend-up' : 'trend-down'}`}>
                            <img className='img-arrow' src={flagBlue ? ArrowUp : ArrowDown} alt=''/>
                        </span>
                    </div> 
                    
                </div>
                <div className='content-dolarofi'>
                    <h3 className='title'>Dólar {DolarOfi}</h3>
                    <div className='content-text'>
                        <h3 className='item-title-venta'>Venta</h3>
                    ...
                    <div ref={divOfieRef} className={`content-text promedio ${flagOfi ? 'higtgreen' : 'lowred'}`}>
                        <h3 className='title-prom'>Promedio</h3>
                        <span className='item-span-prom'>{<CountUp end={averageOfi} duration={0.7}/>}</span>
                        <span className={`content-arrow-ofi ${flagOfi ? 'trend-up' : 'trend-down'}`}>
                            <img className='img-arrow' src={flagOfi ? ArrowUp : ArrowDown} alt=''/>
                        </span>
                    </div> 
